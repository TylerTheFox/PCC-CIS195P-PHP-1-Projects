...
<?php
	...
	if (isset($todo) and $todo == "post") {
		$status = "OK";
		$msg = "<th colspan=2>The following errors were found:</th>
		<tr><td width=15%></td><td>";

		//	if login < 4 char then error
		//
		if (!isset($login) or strlen($login) < 4) {
			$msg = $msg."Login must be at least 4 char<BR>";
			$status = "error";
		}
		...
		if (! $customer->ID() == null) {
			$msg = $msg."Login already exists. Please try another one<BR>";
			$status = "error";
		}
		...
		if (!isset($email)
			...
			$msg = $msg."Email must be valid<BR>";
			$status = "error";
		}
		...
		if (!isset($name) or strlen($name) < 3) {
			$msg = $msg."Name must be at least 3 char<BR>";
			$status = "error";
		}
		...
		if (strlen($password) < 4 ) {
			$msg = $msg."Password must be at least 4 char<BR>$password";
			$status = "error";
		}
		...
		//
		if ($iagree <> 1) {
			$msg = $msg."You must agree to everything<BR>";
			$status = "error";
		}

		if ($status <> "OK") {
			echo "$msg<br><input type='button' value='Retry' onClick='history.go(-1)'></td></td></table>";
		}
		else {
			...
			echo "<tr><td>Welcome $name, You have successfully faked signing up as a new customer with login '$login'.<br /><br />You have not actually been added to the store's customers in this version - so you can only log in as 'lcohen'.<br /><br /><a href = login.php>Click here to login</a><br /></td></tr></table>";
		}
	}
?>
</div>
