...
echo "<p><h2>Test bikes</h2></p>";
$bike = $store->getrental_customerID(1);
echo ($bike->toString());

$bike = $store->getbike_ID(7);
echo '<h3>bike to be added to bike</h3>' . $bike->toString();

$store->addrentalItem(1, 7);

echo "<p><h3>All bikes</h3></p>";
$bikesCollection = $store->getAllbikes();
$bikesCollection->first();
while ($bikesCollection->hasNext()) {
    $bike = $bikesCollection->next();
    echo ($bike->toString());
}


echo "<h3>All bike after bike item added</h3>";
$bikeCollection = $store->getAllbike();
$bikeCollection->first();
while ($bikeCollection->hasNext()) {
    $bike = $bikeCollection->next();
    echo $bike->toString();
