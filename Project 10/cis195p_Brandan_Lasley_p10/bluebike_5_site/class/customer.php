...
class Customer
{
    ...
    public function Customer($id, $email, $name, $male, $iagree, $login, $pass)
    {
        if (isset($id) and !(isset($email) or isset($name) or isset($male) or isset($iagree) or isset($login) or isset($pass))) {
            ...
            $this->login  = null;
            $this->pass   = null;
            echo '<P>===>Error detected: an attempt to create a customer with an unacceptable mix of arguments' . '<br />===>new Customer($id=' . $id . ', $email=' . $email . ', $name=' . $name . ', $male=' . $email . ', $iagree=' . $iagree . ', $login=' . $login . ', $pass=' . $pass . '<br />';
            echo '</P>';
            die();
        }
    }
