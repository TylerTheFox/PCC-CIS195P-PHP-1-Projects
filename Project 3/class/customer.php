...
class Customer  {
	...
	*/
	function Customer($id, $email, $name, $male, $iagree, $login, $pass) {
		...
		//
		if (isset($id) and
				! (isset($email) or isset($name) or isset($male)
						or isset($iagree) or isset($login) or isset($pass))) {
			if ($id == 1) {
				$this->id = 1;
...
/*	for testing only; turn into a // comment to eliminate	*/ //echo "#1.2 ";
		...
		if (isset($login) and isset($pass)
				and ! (isset($id) or isset($email) or isset($name)
							or isset($male) or isset($iagree))) {
			if ($login == 'Brandan' && $pass == sha1('supersecret')) {
					$this->id = 1;
